...
def test_clean_dob():
    # testing good value(s)
    assert clean.clean_dob("04/08/1994") == datetime.date(1994, 8, 4)
...
def test_clean_month():
    # testing good value(s)
    assert clean.clean_month('April 2019') == datetime.date(2019, 4, 1)

    # testing bad value(s)

    assert clean.clean_month(' ') is None
    assert clean.clean_month('', 'Talent/April2019Applicants.csv') == datetime.date(2019, 4, 1)
    assert clean.clean_month(None, 'Talent/June2019Applicants.csv') == datetime.date(2019, 6, 1)
...
def test_clean_invited_by():
    ...
    assert clean.clean_invited_by('fifi eton') == ('Fifi', 'Eton')
    assert clean.clean_invited_by('fifi') == ('Fifi', None)
    assert clean.clean_invited_by('fiFI h ETon') == ('Fifi', 'H Eton')
