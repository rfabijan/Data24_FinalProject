...
def test_extract_json_keys():
    function_return = extractor.extract_json_keys
...
def test_extract_json_name():
    ...
    function_return = extractor.extract_json_name(file)

    assert function_return == 'Stillmann Castano'  # Check return value from file dictionary
    assert extractor.extract_json_name({'name': 'John Doe'}) == 'John Doe'  # Check return value from custom dictionary
    assert extractor.extract_json_name({'age': 1}) is None  # Check return value on invalid dictionary


# function to test extraction of the date from a single JSON file, testing for value of the output
def test_extract_json_date():
    file = extractor.pull_single_json('Talent/10450.json')
    function_return = extractor.extract_json_date(file)

    assert function_return == '15/08/2019'  # Check return value from dictionary inside a valid file
    assert extractor.extract_json_date({'date': '01/01/2021'}) == '01/01/2021'  # Check return value from custom dictionary
    assert extractor.extract_json_date({'dob': '01/01/2021'}) is None  # Check return value from invalid dictionary
...
def test_extract_json_tech_self_score():
    file = extractor.pull_single_json('Talent/10458.json')
    function_return = extractor.extract_json_tech_self_score(file)
    ...
    function_return = extractor.extract_json_tech_self_score({'tech_self_score': {'C++': 1, 'Python': 3}})
    assert len(function_return) == 2
    assert function_return['Python'] == 3
    assert 'C++' in function_return

    # Check invalid dictionary
    function_return = extractor.extract_json_tech_self_score({'strengths': ['Organisation', 'Courteous']})
...
def test_extract_json_json_strengths():
    file = extractor.pull_single_json('Talent/10474.json')
    function_return = extractor.extract_json_strengths(file)
    ...
    # Check custom dictionary
    function_return = extractor.extract_json_strengths({"strengths": ["Programming", "Gaming"]})
    assert len(function_return) == 2
    assert 'Programming' in function_return

    # Check invalid dictionary
    function_return = extractor.extract_json_strengths({'tech_self_score': {'C++': 1, 'Python': 3}})
    assert function_return is None


# function to test extraction of the weaknesses from a single JSON file
# testing for type, length and values of the output
def test_extract_json_weaknesses():
    file = extractor.pull_single_json('Talent/10661.json')
    function_return = extractor.extract_json_weaknesses(file)
    ...
    # Check custom dictionary
    function_return = extractor.extract_json_weaknesses({"weaknesses": ["Procrastination", 'Distractable']})
    assert len(function_return) == 2
    assert 'Distractable' in function_return

    # Check invalid dictionary
    function_return = extractor.extract_json_weaknesses({'tech_self_score': {'C++': 1, 'Python': 3}})
    assert function_return is None


# function to test extraction of the self_development from a single JSON file, testing value of the output
def test_extract_json_self_development():
    file = extractor.pull_single_json('Talent/10836.json')
    function_return = extractor.extract_json_self_development(file)

    # Check dictionary based on valid file
    assert function_return == "No"  # Check return value from dictionary inside a valid file
    assert extractor.extract_json_self_development({"self_development": "Yes"}) == "Yes"  # Custom dictionary as parameter
    assert extractor.extract_json_self_development({"name": "John"}) is None  # Check invalid dictionary as parameter


# function to test extraction of the goe_flex from a single JSON file, testing for value of the output
def test_extract_json_geo_flex():
    file = extractor.pull_single_json('Talent/10887.json')
    function_return = extractor.extract_json_geo_flex(file)

    assert function_return == "Yes"  # Check return value from dictionary inside a valid file
    assert extractor.extract_json_geo_flex({"geo_flex": "No"}) == "No"  # Custom dictionary as parameter
    assert extractor.extract_json_geo_flex({"name": "John"}) is None  # Check invalid dictionary as parameter


# function to test extraction of the financial_support_self from a single JSON file, testing for values of the output
def test_extract_json_financial_support_self():
    file = extractor.pull_single_json('Talent/11040.json')
    function_return = extractor.extract_json_financial_support_self(file)

    assert function_return == "Yes"  # Check return value from dictionary inside a valid file
    assert extractor.extract_json_financial_support_self({"financial_support_self": "No"}) == "No"  # Custom parameter
    assert extractor.extract_json_financial_support_self({"name": "John"}) is None  # Check invalid parameter


# function to test extraction of the result from a single JSON file, testing for return value of the output
def test_extract_json_result():
    file = extractor.pull_single_json('Talent/11098.json')
    function_return = extractor.extract_json_result(file)

    assert function_return == "Pass"  # Check return value from dictionary inside a valid file
    assert extractor.extract_json_result({"result": "Fail"}) == "Fail"  # Custom dictionary as parameter
    assert extractor.extract_json_result({"name": "John"}) is None  # Check invalid parameter


# function to test extraction of the course interest from a single JSON file, testing for return value of the output
def test_extract_json_course_interest():
    file = extractor.pull_single_json('Talent/11579.json')
    function_return = extractor.extract_json_course_interest(file)

    assert function_return == "Data"  # Check return value from dictionary inside a valid file
    assert extractor.extract_json_course_interest({"course_interest": "Engineering"}) == "Engineering"  # Custom parameter
    assert extractor.extract_json_course_interest({"name": "John"}) is None  # Check invalid parameter
